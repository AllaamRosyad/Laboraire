...
class ProductsProvider with ChangeNotifier {
  final CollectionReference productsRef =
      FirebaseFirestore.instance.collection('dataBarang');
  List<ProductModel> products = [];
  ...
  ProductModel? findByProdId(String productId) {
    try {
      return products.firstWhere(
        ...
        orElse: () => ProductModel(
          productId: '',
          productName: 'Unknown', // Corrected field name
          productDescription: '',
          productImage: '',
          productStock: 0, // Provide a default value for stock
  ...
  List<ProductModel> searchQuery({
    ...
    return passedList
        .where((element) => element.productName
            .toLowerCase()
            .contains(searchText.toLowerCase())) // Use productName here
        .toList();
  }

  // Fetch products from Firestore for users
  Future<void> fetchProducts() async {
    try {
      final productSnapshot = await productsRef.orderBy('createdAt').get();
      products.clear();
      for (var element in productSnapshot.docs) {
